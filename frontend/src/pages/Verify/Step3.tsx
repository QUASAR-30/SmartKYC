...
const Step3NFI: React.FC<StepProps> = ({ setFormData, nextStep, prevStep, formData }) => {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      setFormData(prev => ({ ...prev, nifFile: e.dataTransfer.files[0] }));
    }
  }, [setFormData]);

  return (
    <div className="p-8 max-w-lg border border-yellow-600 mx-auto bg-white rounded-2xl shadow-md">
      <h2 className="text-2xl font-semibold mb-2">Étape 3/5 : Numéro d’Identification Fiscale (NIF)</h2>
      <p className="text-gray-600 mb-6">
        Téléchargez une image ou un PDF de votre attestation NIF délivrée par l’administration fiscale.
      </p>
...
>
      ...
      {formData.nifFile && (
        <div className="mt-4 p-3 bg-green-50 rounded-lg flex items-center">
          ...
          <span className="text-sm text-green-700 truncate">{formData.nifFile.name}</span>
        </div>
      )}
